...
#' @param k1 the first rate constant, expressed in reciprocal of the X axis time units. The first half-life is 0.6932/k1.
#' @param k2 the second rate constant, expressed in reciprocal of the X axis time units. The second half-life is 0.6932/k2.
#' @param range1 the range of y in the first phase of decay.
#' @param range2 the range of y in the second phase of decay.
...
#' set.seed(2345)
#'
#' decay2dat <- simdecay2(time, k1 = 0.5, k2 = 0.2, range1 = 2, range2 = 10, ymin = 1.0, cv = 0.10, reps = 5); decay2dat
#'
#' decay2dat$data
#'
#'
simdecay2 <- function(x, k1, k2, range1, range2, ymin, cv=0.10, reps=5) {

  yp <- range1*exp(-k1*x) + range2*exp(-k2*x) + ymin
  ...
  ggplot2::ggplot(values,
    ...
    ggplot2::labs(title="model: y = range1*exp(-k1*x) + range2*exp(-k2*x) + ymin") +
    ggplot2::geom_smooth(
      method=minpack.lm::nlsLM,
      formula = "y ~range1*exp(-k1*x) + range2*exp(-k2*x) + ymin",
      method.args = list(start=c(range1=range1, range2=range2, k1=k1, k2=k2, ymin=ymin)), se=F, color="blue")}

simdecay2(x = 1:20, k1 = 1, k2 = 0.2, range1 = 2, range2 = 10, ymin = 1, cv=0.20, reps=5)
